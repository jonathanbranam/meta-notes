...
- [ ] 3.1 Document `week_quarter` in `doc/meta-notes.txt` under "Other variables", saying to use it for week plan paths and showing the daily template's link as the example; verify `:help meta-notes-template-variables` shows it

## 4. Verification

- [ ] 4.1 Run `./run_tests.sh` and `pipenv run pytest test/unit/`; all pass
- [ ] 4.2 In a scratch notes root, run `meta-notes note daily 2026-04-02` and `meta-notes note weekly 2026-04-02`; verify the daily note's week link, with `.md` appended, is the path the weekly command printed
- [ ] 4.3 Run `openspec validate fix-week-plan-link-quarter --strict`; it passes
