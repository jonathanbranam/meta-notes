...
Every `#tag` on a task line SHALL be a tag of the task, wherever it appears in the line. A tag is `#` followed by letters, digits, `_`, or `-`. The tags `#mtg`, `#pers`, and `#per` SHALL be read as `meeting`, `personal`, and `personal`, the same aliases that time log entries use. Tags SHALL be compared ignoring case.
...
**Reason**: The options and output of `find_tasks.py` change in this release (task model, selection modes, removed `--due-*` options, no week buckets).
**Migration**: `find_tasks.py` stays runnable directly, and "Task query matches find_tasks" keeps it and `meta-notes tasks` in step. Replace `--due-on D` with `--due --date D`, `--due-by D` with `--overdue --date D+1`, and `--due-between A B` with `--due --date A..B` in templates.
