...
- **THEN** the result SHALL have `tag` `make-bread`, `has_next` true, `last_review` null, `since` `2026-06-27`, `review-overdue` in `warnings`, and the `#next` task in `open` with its file, line, and text
