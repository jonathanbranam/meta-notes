...
- [ ] 3.2 Check that README's Planning Skills table and the `ceremony-skills` Purpose name `project-review`; verify by reading both after archiving syncs the spec

## 4. Verification

- [ ] 4.1 In a scratch notes root with a never-reviewed project, a never-reviewed `done` project, a reviewed one, and one with a `#review` due after today, walk through `project-review` with a fixed date and verify it picks the never-reviewed active project, every edit went through `task update` or a confirmed command, and the home note gains a completed `#review` line
- [ ] 4.2 In the same root, walk through done with archive and verify the `status` and `#review` lines are written before `meta-notes archive` runs and appear in `archive/project/`
- [ ] 4.3 In the same root, stop a review before the disposition and verify the home note has a completed `#review` for today and an open `#review` dated the next workday
- [ ] 4.4 Run `openspec validate project-review-skill --strict` and verify it passes

## 5. Release

- [ ] 5.1 Bump `__version__` PATCH in `scripts/meta_notes/__init__.py` when archiving the change, tag `v<version>`; verify `meta-notes --version`
